...
optional<Register> SM72445_I2C::read(
	...
	auto resultCode = HAL_I2C_Mem_Read(
		this->hi2c,								  //
		static_cast<uint8_t>(deviceAddress) << 1, // I2C Address. Shift required by HAL.
...
optional<Register> SM72445_I2C::write(
	DeviceAddress deviceAddress,
	MemoryAddress memoryAddress,
	Register	  data
) {
	...
	dataBuffer[0] = 7; // Fixed length of all frames sent to SM72445.

	for (auto i = 0u; i < sizeof(dataBuffer) - 1; i++) {
		dataBuffer[i + 1] = static_cast<uint8_t>((data >> (i * 8)) & 0xFFu);
	}
